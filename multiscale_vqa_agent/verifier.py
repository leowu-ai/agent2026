...
A WSI-derived categorical ER/PR/HER2 prediction is valid evidence for a compatible categorical benchmark target. The active HER2 prototype is trained specifically for the four categorical scores 0, 1+, 2+, and 3+, so its predicted score is direct predictive evidence for a matching score target and may also support general status using 0/1+=negative, 2+=equivocal, and 3+=positive. Each remains a WSI-derived prediction, not a measured assay, and cannot answer an exact percentage, FISH/ISH ratio, amplification, copy number, test-performed, or report-specific question.
