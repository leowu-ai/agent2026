...
COMPLETE means target evidence resolves the MCQ's core semantic distinction. Examples: grade 1/2/3 with histologic grade; yes/no LVI with LVI; and HER2 score choices 0/1+/2+/3+ with the HER2 prototype whose catalog labels are those same four scores.
...
Assay granularity matters. General ER/PR status is target_evidence/complete. The active HER2 prototype is explicitly trained for categorical IHC scores 0, 1+, 2+, and 3+, so a four-level HER2 score question is target_evidence/complete. For general HER2 status, interpret 0/1+ as negative, 2+ as equivocal, and 3+ as positive; if an equivocal option is absent, a 2+ prediction is only partial evidence. ER status for choices negative, positive <10%, or positive >10% is target_evidence/partial because it separates negative from positive but not percentage ranges. HER2 questions about FISH/ISH, gene amplification, exact copy number, which test was performed, or report wording are correlated_context or none, never partial. Receptor predictions for which stain was performed or pending are correlated_context.
